...
# batch correct
pheno <- pData(prevail.eset)
edata <- exprs(prevail.eset)
batch = prevail.eset$Batch
modcombat = model.matrix(~1, data=pheno)
combat_edata = ComBat(dat=edata, batch=batch, mod=modcombat, par.prior=TRUE, prior.plots=FALSE)
corrected.eset <- prevail.eset
exprs(corrected.eset) <- combat_edata

# filter
clean.eset <- varFilter(corrected.eset, var.cutoff = 0.5)
# early = Day 1 late = Day 14,21,28
clean.eset <- clean.eset[ , clean.eset$code=='A']

# subet according to days 
...
fit <- lmFit(exprs(EarlyLate.eset), design)
